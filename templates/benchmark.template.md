...
> **本文件由 `/cheat-essay-learn-from` 创建和维护**。
>
...
>
> 你随时可以：
> - **追加**对标文章：`/cheat-essay-learn-from --append <new-videos>`
> - **替换**对标账号：`/cheat-essay-learn-from --replace <new-account>`
...
| # | 文章标题 | 阅读 | 在看 | 点赞 | 分享 | 你的印象 | transcript |
|---|---|---|---|---|---|---|---|
...
> **印象档**：高 / 中 / 低 是你看完这篇文章后**主观**判断的"算不算这个账号的代表作"——不是数据驱动，是直觉判断。这个判断比数据更能告诉 Claude 你想做什么风格。
...
> cheat-essay-seed brainstorm 时会读这里——根据对标账号的主题分布给提议。
...
> cheat-essay-seed Mode A/B/C 仍以你的实际想做什么为主。
...
- 当 `state.calibration_samples >= 10` 时，cheat-essay-status 会提示"你已有足够自己数据，benchmark 主要作 sanity check 不再主导"
- 但 benchmark.md **不删**——它仍是 cheat-essay-seed brainstorm 的 reference frame
- 如果对标账号本身风格变了 / 你不想再对标了 → 跑 `/cheat-essay-learn-from --replace <none>` 解除（保留历史在文件，但 cheat-essay-seed 不再读）
