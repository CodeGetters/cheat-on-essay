#!/usr/bin/env bash
#
# cheat-on-essay / prediction-immutability hook
#
...
if [[ "${CHEAT_BYPASS_IMMUTABILITY:-0}" == "1" ]]; then
  echo "[cheat-on-essay] ⚠️  IMMUTABILITY BYPASS active (CHEAT_BYPASS_IMMUTABILITY=1)" >&2
  echo "[cheat-on-essay] ⚠️  This should only be used for pure markdown-formatting fixes." >&2
  echo "[cheat-on-essay] ⚠️  Bypass will be visible in git history." >&2
  exit 0
fi
...
[cheat-on-essay] 🚫 BLOCKED: edit targets the '## 预测' / '## Prediction' section of:
  $file_path

This violates principle #1 of cheat-on-essay: predictions are immutable.
...
[cheat-on-essay] 🚫 BLOCKED: Write would overwrite an existing prediction file:
  $file_path
