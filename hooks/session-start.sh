#!/usr/bin/env bash
#
# cheat-on-essay SessionStart hook
#
...
#
# Silently exits if:
#   - Not in a cheat-on-essay project (no .cheat-state.json)
...
# Silently skip if not a cheat-on-essay project
...
if ! command -v jq >/dev/null 2>&1; then
  cat <<'EOF'
[cheat-on-essay] SessionStart: jq not installed — skipping auto status report.
...
if [[ "$target_cadence" == "null" ]] || [[ -z "$target_cadence" ]]; then
  ...
  if   [[ $buffer_days -lt 1 ]]; then
    ...
    buffer_warning="🚨 buffer 警戒：下个发布日可能断更。今天必须写 ≥1 篇稳分。"
    ...
    buffer_warning="📦 buffer 积压：建议暂停写作，先发存货 + 复盘。"
  fi
fi
...
# --- Build the report ---
echo ""
echo "[cheat-on-essay / SessionStart 状态报告]"
echo ""
echo "$buffer_label"
echo "$retro_label"
