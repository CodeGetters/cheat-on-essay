...
> 💡 调用时覆盖：`/cheat-essay-score draft.md — OUTPUT_DETAIL: compact`
...
1. 读 `.cheat-state.json` → 不存在则提示用户先跑 `/cheat-essay-init`，停止
...
**打分速度纪律**（参考 starter-rubrics/wechat-essay.md 的 cheat sheet）：
...
5. **不算 candidate composite**。candidates.md 里的 composite 字段在 cheat-trends/cheat-essay-recommend 里写——score 只服务"已写好的具体稿子"

## Refusals

- 「打分顺便预测一下」 → 拒绝。请改用 `/cheat-essay-predict`。原因：predict 必须走 blind check + 写 immutable 日志，score 跳过这些
...
- 是 `cheat-essay-predict` 的前置探索：用户可以反复 score 不同稿子版本，确定一份再 predict
