---
name: cheat-essay-seed
description: 跟用户对话讨论选题——**默认一次一个**，用户主动给主题或经历，AI 围绕用户的输入深挖、提炼角度、写一份 draft。不是 AI 拿三个开放问题追用户，也不是一次给 5 个候选。触发词："找选题"/"我想做一条 X"/"最近有个想法"/"seed"/"启动种子"。可选 batch 模式：`/cheat-essay-seed --batch 5` 走旧的 brainstorm 5 候选 + 写 5 draft 流程。
argument-hint: [— batch: N] [— sources: <comma-separated>]
allowed-tools: Bash(*), Read, Write, Edit, Glob, WebFetch, Skill
---

# /cheat-essay-seed — 选题对话（默认）/ 批量 brainstorm（可选）
...
```
Mode A — 用户主动给主题（**最常见**）：
  用户："/cheat-essay-seed" + 直接说"我想做一条关于 X 的"
       或："/cheat-essay-seed 我最近开会被领导..."
  ↓
...
Batch Mode — 用户显式要批量（`/cheat-essay-seed --batch 5`）：
...
1. 读 `.cheat-state.json` → 不存在则提示先跑 `/cheat-essay-init`
...
- **B 缺席**（benchmark 为空）+ 用户数据少：纯靠用户 input + 抓热点；明确告诉用户"没 benchmark 也没足够自己数据，建议跑 /cheat-essay-learn-from 后再回来 brainstorm"
...
**纯 `/cheat-essay-seed` 无附加内容** → **询问入口问题**：
...
注意这是**唯一的开放式问题**——只在用户**纯触发** `/cheat-essay-seed` 时问。如果用户已经在触发词里给了内容（"/cheat-essay-seed 我想做..." 或 "找选题 我最近开会..."），直接进 Mode A/B/C 不再问这一句。
...
- 触发瞬间："你说 X 这件事，最初是哪个具体瞬间触发你想做的？" / "是什么让你觉得这值得写一篇文章？"
...
1. **拉外部素材**（按 `content_form` 选 trend source）：
   ...
   - `opinion-essay` / `knowledge-deep` / `personal-narrative` / `other` → 调 trendradar-mcp（如启用）
   - `mixed` → 两个都调
...
`WITH_DRAFT=yes` → 顺次写到 `drafts/<YYYY-MM-DD>_<id>_<short-title>.md`：
...
> 改写流程：
> 1. **直接在本文件改写**（同 path：drafts/<...>.md）
>    - 加你的语气、个人经历、真实金句
>    - 砍铺垫、砍模型缩写、砍学术包装
> 2. 改完后跑 `/cheat-essay-predict drafts/<本文件>.md`
> 3. 定稿后跑 `/cheat-essay-finalize drafts/<本文件>.md`
...
```
✅ Draft 写完：drafts/2026-05-04_<id>_<short>.md

接下来你可以：
- 改写这份 draft（直接在原文件改）
- 改完跑 "打分这篇 drafts/<...>.md" 看 7 维评分
- 决定要发 → "启动预测 drafts/<...>.md"
...
1. **AI 不主动开放问**——只在用户纯触发 `/cheat-essay-seed` 时问一次入口问题，其他时候**等用户给输入再深挖**
...
- 上游：`/cheat-essay-init` Phase 5 末尾在 `pool_status=none + calibration_samples=0` 时主动询问"现在跑 /cheat-essay-seed？"
- 上游：`/cheat-essay-recommend` 在 candidates 空时引导文案中提及 `/cheat-essay-seed`
- 上游：`/cheat-essay-status` 在 `pool_status=none + 距 init >24h` 时提示"还没拍——跑 /cheat-essay-seed？"
- 下游：用户的 candidate → candidates.md（tier1，已 deep_read）
- 下游：（默认）draft → drafts/<id>.md → 用户改写 → /cheat-essay-predict
- 与 `/cheat-essay-trends` 区别：cheat-seed 是**讨论 + 写 draft**（重 conversation）；cheat-trends 是**多 adapter 抓 + 粗打分**（重 fetch）。两者目的不同，不互相替代。
