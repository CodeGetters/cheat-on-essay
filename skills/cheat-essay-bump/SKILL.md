...
# /cheat-essay-bump — Rubric / Bucket 升级
...
每个样本：
  ...
  actual_plays_rank: 用实际阅读数排序的 rank
...
`/cheat-essay-bump --bucket-only [--scheme ratio|absolute|percentile]`
...
下一次 `/cheat-essay-predict` 起按新 bucket 派生。历史 prediction 文件里的 bucket 标签**不重算**——bucket 是预测时的语义判断，事后改写会破坏盲度。
