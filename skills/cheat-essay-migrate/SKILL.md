...
# /cheat-essay-migrate — Schema 版本迁移

把用户 `.cheat-state.json` 从旧 `schema_version` 升级到 cheat-on-essay 当前期望的 `LATEST_SCHEMA`。
...
> 💡 调用时覆盖：`/cheat-essay-migrate — dry-run: false` 直接执行 / `/cheat-essay-migrate — to: 1.2` 仅升到指定版本
...
1. 输出 "→ [N/M] 应用 {file}..."
2. 读 `migrations/<file>` → 找到 `## HOW (Claude steps for /cheat-essay-migrate)` 段
...
5. 如某步失败：
   ...
   - 提示："已停在 schema_version: {last_success_version}。修复后重跑 /cheat-essay-migrate 会从这里继续"
   - 退出
...
📦 备份保留：.cheat-state.json.backup-1714838400
   （确认一切正常后可手动 rm；下次成功 /cheat-essay-init 也会清理过期备份）

下一步建议：
  - 跑 /cheat-essay-status 确认看板正常
...
6. **不动 predictions / rubric / articles**：只改 `.cheat-state.json`。其他用户数据由各自 skill 负责，迁移 skill 不碰
...
- 上游：SessionStart hook 检测 `state.schema_version != LATEST_SCHEMA` → 输出红色警告 + 建议跑 `/cheat-essay-migrate`
- 上游（手动）：用户 git pull 拉了新版后，看 CHANGELOG 标 BREAKING → 主动跑
- 下游：跑完后所有其他 skill 读 state 都能拿到最新字段
- 与 `cheat-essay-init`：init 写新 state 时直接用 LATEST_SCHEMA，不需要走 migrate
...
修复 .cheat-state.json 后重跑 /cheat-essay-migrate 会从 1.2 → 1.3 继续。
```
