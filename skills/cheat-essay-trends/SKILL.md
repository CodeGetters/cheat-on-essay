---
name: cheat-essay-trends
description: 从配置的热点源（微博热搜 / 知乎热榜 / 公众号看一看热门 / TrendRadar / 等）抓今天的热门话题，去重 + 粗打分 + 写入 candidates.md。**绝大部分人没有候选池——这是让"我没素材"问题在 onboarding 第二步就消失的钥匙**。触发词："抓热点"/"fetch trends"/"今天有什么可做的"/"trending now"/"找选题"。
argument-hint: [— sources: <comma-separated>] [— max-per: 20]
allowed-tools: Bash(*), Read, Write, Edit, Glob, WebFetch, Skill
---

# /cheat-essay-trends — 热点抓取

多 adapter 模式：读各 `trend-sources` adapter 的输出 → 去重 → 粗打分 → 写入 `candidates.md`。

> **v0.1.0 状态**：所有热点源标记为 `experimental`。抓取实现已迁移自 cheat-on-content（视频版），公众号场景下的命中率和噪声比例**尚未真实校准**。如果你跑出来的 candidates 质量差，先 raise issue，再降低使用频率。
...
[Phase 3: 去重（vs candidates / predictions / trends-history）]
  ↓
[Phase 4: 对每个新 item 粗打分（调 cheat-essay-score 内联逻辑）]
  ↓
...
- **DEDUPE = true** — 去重开关
- **AUTO_SCORE = true** — 抓回来后自动调 cheat-essay-score 粗打分
- **MIN_COMPOSITE_TO_SUGGEST = 6.0** — 低于此分的不推荐用户加入候选池（仍写入 trends-history 避免下次重复推）

> 💡 调用时覆盖：`/cheat-essay-trends — sources: manual-paste,weibo-hot,zhihu-hot — max-per: 10`
...
最快配法：
- 临时跑：/cheat-essay-trends — sources: manual-paste,weibo-hot
- 永久启用：编辑 .cheat-state.json 的 enabled_trend_sources 数组

可用 adapter（详见 adapters/trend-sources/，**全部标 experimental**）：
- manual-paste（默认，永远能用）
- weibo-hot（微博热搜，无需 key）
- zhihu-hot（知乎热榜，无需 key）
- wechat-hot（公众号看一看热门 / 第三方聚合，部分需 cookie）
- aihot（AI 相关聚合，需配 endpoint）
- trendradar-mcp（TrendRadar MCP 服务，需先配 MCP）
- thirdparty-paid（新榜 / 西瓜，需自己接 API）
```
...
| Adapter | 实现机制 | 状态 |
|---|---|---|
| `manual-paste` | 询问用户："粘贴你今天的候选 URL/标题列表（每行一条）" → 解析每行，对 URL 做 WebFetch 拓展 snippet | ✅ 稳定 |
| `weibo-hot` | WebFetch 微博热搜聚合接口（公开） → 提取 title/url/snippet | ⚠️ experimental |
| `zhihu-hot` | WebFetch 知乎热榜 API（公开） | ⚠️ experimental |
| `wechat-hot` | 公众号生态没有公开热门 API，依赖第三方聚合（如新榜 / 微小宝），部分需 cookie/key | ⚠️ experimental |
| `aihot` | AI 相关聚合源 | ⚠️ experimental |
| `trendradar-mcp` | 通过已配的 TrendRadar MCP 服务拉数据 | ⚠️ experimental |
| `thirdparty-paid` | schema only——读 `adapters/trend-sources/thirdparty-paid.md`，让用户自己接 | ⚠️ experimental |
...
✅ manual-paste: 5 条
✅ weibo-hot: 18 条
⚠️  wechat-hot: 跳过（缺第三方 API key——配置见 adapters/trend-sources/wechat-hot.md）
✅ zhihu-hot: 15 条
```
...
2. 按当前 rubric 给 7 维打分（**不**调 `/cheat-essay-score` 子 skill 走 IO；inline 复用打分逻辑）
3. 算 composite
4. 给一句 rationale

**注意**：粗打分 ≠ 正式预测。预测必须基于最终稿（用户改过的），这里的打分只是"是否值得展开写"的粗筛。

`AUTO_SCORE=false` 时，items 写入 candidates.md 时 composite=null，需要后续手动 `/cheat-essay-score`。
...
|---|---|---|---|---|---|
| 1 | 为什么我们都讨厌主动联系朋友 | weibo-hot | 8.4 | 1w-10w | IE+QD 双 5，AB 普适 |
| 2 | "她不一样"的一千种变体 | zhihu-hot | 8.1 | 1w-10w | EG 颗粒度强 |
| 3 | ...... |
...
6. **公众号场景的热点源命中率未校准**——所有 experimental adapter 的输出可能噪声大，用户应有心理预期

## Refusals

- 「直接抓微信看一看 feed，不用 cookie」 → 拒绝。微信看一看无公开端点，无 cookie 必失败；引导到 wechat-hot adapter 配置文档（或第三方聚合 API）
- 「跳过去重，把所有抓到的都写进去」 → 拒绝。会污染候选池，下次 recommend 时排序失效
- 「跳过粗打分，直接写 raw 标题」 → 允许（`AUTO_SCORE=false`），但提示用户后续需要 `/cheat-essay-score` 才能进 recommend 池

## Integration

- 上游：用户配置 `.cheat-state.json` 的 `enabled_trend_sources` 数组
- 下游：`/cheat-essay-recommend` 直接读 `candidates.md` 排序——trends 写完，recommend 立刻看到
- 与 `/cheat-essay-init`：onboarding Q4 选"没有候选池"的用户被引导到这里
- 与 `/cheat-essay-status`：status 看板显示"上次抓热点：X 天前 / 待清理候选池：Y 条"
...
4. **失败模式**：常见错误 + 优雅降级行为
5. **稳定性等级**：★ 1-5 颗星
6. **v0.1.0 状态**：本仓库所有非 manual-paste adapter 均为 experimental，从 cheat-on-content 继承，公众号场景的真实命中率未校准
