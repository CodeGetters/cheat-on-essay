...
被这些子 skill 引用：`cheat-essay-predict`、`cheat-essay-retro`、`templates/prediction.template.md`。
...
**Article ID**: <12 位 hash>  (sha256 of drafts/<id>.md initial content, 取前 12)
**Title**: <作品完整标题>
**Rubric Version**: **v0** | **v1** | **v2** | ...
**预测时间**: 2026-05-04（基于最终稿）
**Script Path**: drafts/2026-05-04_<id>_<short>.md
...
- `Script Path` 必填——指向 `drafts/<id>.md`（pre-shoot 草稿）
- `Script Hash` 必填——cheat-essay-finalize 时再 hash `articles/<id>/script.md`，不一致 → 复盘段加 integrity warning
...
## 预测 v2 (replaces v1)        ← (可选) 拍后改稿 ≥30% 时由 cheat-essay-finalize 触发，append 不覆盖
...
- **新建文件**：cheat-essay-predict 写 `## 预测 v1`（不再裸 `## 预测`——为 v2 留 schema 一致性）
- **legacy 兼容**：v0.1.0 时期写的 `## 预测` 文件不动；hook 与 cheat-essay-retro 都识别
- **v2 触发条件**：cheat-essay-finalize 检测拍摄稿与 `drafts/<id>.md` 的 line-diff ≥ 30%（[V2_TRIGGER_THRESHOLD](../skills/cheat-essay-finalize/SKILL.md)），调用 `/cheat-essay-predict — mode: v2 — prediction-file: <path>`
- **append 而非覆盖**：v2 段插在 `## 复盘` 之前。v1 段**绝不**修改（hook 物理强制）
- **校准用谁**：cheat-essay-retro 读最后一个 `## 预测 vN` 算偏差；v1 留作历史档案
...
cheat-essay-retro 用此字段在 score-curve / bump 校准时区分两条数据线，避免混样。

---

## 子 skill 验收标准

`cheat-essay-predict` 写完一份预测后，必须自检 7 个组件齐全：
- 组件 5 / 7 在校准样本不足时仍写"N/A 段 + 解释"，**不允许直接跳过**
- header 的 `Calibration Samples` + `Confidence` 必填——读者一眼看到这次预测多可信

`cheat-essay-retro` 写复盘段时，必须先校验该文件的 7 个组件：
- 缺组件 → 警告"该 prediction 不规范，复盘价值打折"
- 复盘段格式与 confidence 等级**无关**——任何阶段复盘都是同一格式
- diff `Script Hash` 与当前 `articles/<id>/script.md` 的 hash → 不一致则在复盘段加 `**Script changed between predict and shoot**` 警告
