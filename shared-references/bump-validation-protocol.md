...
被这些子 skill 引用：`cheat-essay-bump`、主 SKILL.md。
...
`CROSS_MODEL_AUDIT=false`：跳过外部审核——**仅在离线/无网时使用**。state file 标记 `last_bump_self_audited: true`，`cheat-essay-status` 持续提示用户配置外部审核。
...
2. 在 `rubric_notes.md` 的"vN → vN+1 升级 Memo" 段写完整 memo（触发观察 + 证据数据 + 诊断 + 新公式 + 已知局限）
...
| Step 4 外部审核 REJECT | 把外部 LLM 的理由完整记录到 `rubric_notes.md` 的"被拒升级 log"段 |
