...
被这些子 skill 引用：`cheat-essay-predict`、`cheat-essay-retro`、主 SKILL.md。
...
子 skill 在启动 `cheat-essay-predict` 前，必须自检并向用户**主动声明**：
...
```
drafts/<date>_<id>_<short>.md        ← pre-shoot 草稿（cheat-essay-seed 写或用户写）
predictions/<date>_<id>_<short>.md    ← immutable 预测（cheat-essay-predict 写）
articles/<date>_<id>_<short>/           ← 拍后才建（cheat-essay-finalize 创建）
  ├── script.md                       ← 用户提供的最终拍摄稿
  └── report.md                       ← T+3d 数据（cheat-essay-retro 写）
```

- `<date>`：**草稿首次落盘日期**（即 `drafts/<id>.md` 的创建日），不是预测日 / 拍摄日 / 发布日。理由：保持 ID 稳定——草稿大改后 hash 变了仍想保持文件可追溯
...
- `drafts/<date>_<id>_<short>_redo.md`
- `predictions/<date>_<id>_<short>_redo.md`
- `articles/<date>_<id>_<short>_redo/`
...
| 场景 | 处理 |
|---|---|
| 预测文件不小心被人手编辑了预测段 | 不自动回滚（破坏更大）。下次 `cheat-essay-retro` 检测到不一致 → 在复盘段追加 `**Integrity warning**: 预测段于 <ISO timestamp> 被外部修改，无法保证盲度`，校准价值降级为"参考"，不计入 bump 校准池 |
...
盲预测是整个 cheat-on-essay 校准循环的**唯一信号源**。一旦预测段被事后修改，所有"哪个维度被验证 / 推翻"的判断都失去基线——你不知道当初是真预测对了，还是事后改对了。
