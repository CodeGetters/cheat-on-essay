...
被这些子 skill 引用：`cheat-essay-trends`、`cheat-essay-recommend`、`cheat-essay-init`、所有 `adapters/`。
...
打分前 score 字段全部为 null——是预期的。`cheat-essay-trends` 抓回来后会调 `cheat-essay-score` 给每条算 composite。
...
升级到 SQLite 之后（见 `cheat-essay-status` 的升级触发），同样字段走 `articles` 表存储，markdown 视图自动从 DB 渲染。
...
`cheat-essay-trends` / `cheat-essay-recommend` 在写入 `candidates.md` 前必须执行：
...
| Tier | 含义 | 对应行动 |
|---|---|---|
| `tier1` | 强候选，应推荐 | 进入 `cheat-essay-recommend` 排序池 |
...
**Cold-start 期间所有 item 默认是 `unread`/`null tier`**——直到用户或 `cheat-essay-score` 给出 composite 后才能粗分类。**未打分的 item 不应出现在 `cheat-essay-recommend` 输出**——避免推荐没读过的素材。
