...
被 `cheat-essay-migrate` skill / `cheat-essay-init` / SessionStart hook / 维护者引用。规定如何安全演进 `.cheat-state.json` schema 而不让老用户被打断。
...
- **改 enum 取值**（如 `"opinion-essay"` 改成 `"opinion_video"`，老值不再合法）
- **拆字段 / 合字段**
...
### 1. 改 cheat-essay-init 写新 state 的硬编码 schema_version
...
位置：`skills/cheat-essay-init/SKILL.md` Phase 3 的 state 写入段。
...
- WHAT changed
- WHY
- HOW (Claude steps for /cheat-essay-migrate)
- Manual fallback
...
if [[ "$state_schema" != "$LATEST_SCHEMA" ]]; then
  ...
  echo "   建议跑 /cheat-essay-migrate 升级（不阻塞继续工作）"
fi
```
...
`/cheat-essay-migrate` 写之前会备份到 `.cheat-state.json.backup-<timestamp>`。

备份保留多久：
- 用户跑 `/cheat-essay-status` 时，如果有备份 + state 已稳定运行 N 天 → 提示"可以清理 N 个旧备份"
- `/cheat-essay-init` 重 init 时清理所有旧备份（既然要重 init，老备份意义不大）
