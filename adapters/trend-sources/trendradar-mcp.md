...
cheat-on-essay 把它当作 trend-sources adapter 之一——用户配 MCP server 后，cheat-seed / cheat-trends 自然能调。
...
cheat-on-essay 不打包 TrendRadar——用户自己装、自己保管 server 资源。
...
如果你是 TrendRadar 维护者看到这份 adapter doc——感谢你把多平台聚合做成 MCP server。cheat-on-essay 是你们项目的"内容生产侧下游"——用户用 TrendRadar 知道发生了啥，用 cheat-on-essay 把这个变成可校准的内容预测循环。互补不替代。

欢迎 cross-link：[github.com/<your-org>/cheat-on-essay](https://github.com/<your-org>/cheat-on-essay)。
