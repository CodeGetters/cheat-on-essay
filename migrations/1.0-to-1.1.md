...
| 字段 | 默认值 | 写入者 |
|---|---|---|
| `typical_duration_seconds` | 240 | cheat-essay-init Q1.5 派生 |
| `target_publish_cadence_days` | null（灵活节奏） | cheat-essay-init Q1.6 派生 |
| `rubric_form_mismatch` | false | cheat-init（content_form ≠ opinion-essay 时 true） |
| `benchmark_status` | "none" | cheat-essay-init Phase 2.5 |
| `benchmark_name` | null | cheat-essay-learn-from |
| `benchmark_sample_count` | 0 | cheat-essay-learn-from |
| `baseline_plays` | null | cheat-essay-init / cheat-essay-retro |
| `enabled_perf_adapters` | `[]` | cheat-essay-init |
| `last_published_file` | null | cheat-essay-publish |
| `last_retro_at` | null | cheat-essay-retro |
| `pending_retros` | `[]` | cheat-essay-publish / cheat-essay-retro |
| `shoots` | `[]` | cheat-essay-finalize / cheat-essay-publish |
...
3. 派生新字段值：
   ...
   - `rubric_form_mismatch`: 检查 content_form 是否 ≠ "opinion-essay"（v1.0 字段保留）→ 是则 true
   ...
   - `shoots`: 扫 `articles/` 找有 script.md 但对应 prediction 无 `published_at` 的 → 构造 shoot 条目
...
不想跑 `/cheat-essay-migrate` 的用户可以手改 `.cheat-state.json`：
...
> 手改后建议跑 `/cheat-essay-status` 验证 state 文件能被所有 skill 正常读。
