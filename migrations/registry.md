...
cheat-on-essay 的 schema 版本演进单一来源。`/cheat-essay-migrate` 读这份文件决定要跑哪些迁移。

---

## 当前 schema_version

**`1.2`** —— 由 `cheat-essay-init` Phase 3 写入新 state 文件。

下方 `LATEST_SCHEMA` 标记位被 `cheat-essay-migrate` skill 和 SessionStart hook 引用：
...
按时间顺序，每行表示一个 schema 升级。`/cheat-essay-migrate` 用此表算出从用户当前版本到 LATEST_SCHEMA 需要按顺序跑哪些 step。
...
- 每份必含 4 段：
  ...
  2. **WHY** — 为什么这个改动
  3. **HOW (Claude steps)** — Claude 跑 `/cheat-essay-migrate` 时按顺序执行的自然语言步骤
...
7. 跑一遍：在 fixture 老 state 上调 `/cheat-essay-migrate`，验证升级到新版且 state 字段齐全
