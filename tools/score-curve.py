#!/usr/bin/env python3
"""
score-curve.py — predict accuracy convergence chart for cheat-on-essay.

Reads predictions/*.md (in the user's project), pairs each prediction's
center-of-bucket estimate against actual reads from the retrospective section,
...
# explicitly). Units: 万 (10,000 reads). Adjust per account size if needed.
BUCKET_CENTERS = {
    "<1000": 0.05,
    "1000-5000": 0.3,
    "5000-1w": 0.75,
...
ACTUAL_READS_RE = re.compile(r"阅读[：:]\s*\*?\*?(\d+(?:\.\d+)?)\s*w", re.IGNORECASE)
...
class Sample:
    ...
    actual_reads_w: Optional[float]

    @property
    def has_retro(self) -> bool:
        return self.actual_reads_w is not None

    @property
    def signed_error_pct(self) -> Optional[float]:
        """(actual - predicted) / predicted, in percent."""
        if self.predicted_center_w is None or self.actual_reads_w is None or self.predicted_center_w == 0:
            return None
        return (self.actual_reads_w - self.predicted_center_w) / self.predicted_center_w * 100
...
def parse_prediction_file(path: Path) -> Sample:
    ...
    if retro_section.strip():
        am = ACTUAL_READS_RE.search(retro_section)
        if am:
            actual_reads_w = float(am.group(1))
...
def render_chart(samples: list[Sample], out_path: Path, window: int) -> bool:
    ...
    ax.set_title("Cheat-on-Essay — 预测精度收敛曲线")
...
def render_csv(samples: list[Sample]) -> None:
    ...
    writer.writerow(["file", "date", "bucket", "predicted_center_w", "actual_reads_w", "signed_error_pct"])
    for s in sorted(samples, key=lambda x: x.date):
        writer.writerow(
            [
                ...
                s.actual_reads_w if s.actual_reads_w is not None else "",
