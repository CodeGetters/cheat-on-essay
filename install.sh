...
# → /cheat-essay-init runs the onboarding.
#
...
SUB_SKILLS=(
  cheat-essay-init
  cheat-essay-learn-from
  cheat-essay-seed
  cheat-essay-score
  cheat-essay-quotes
  cheat-essay-predict
  cheat-essay-finalize
  cheat-essay-publish
  cheat-essay-retro
  cheat-essay-bump
  cheat-essay-recommend
  cheat-essay-trends
  cheat-essay-status
  cheat-essay-migrate
)
...
CODEX_SKILLS=(cheat-on-essay "${SUB_SKILLS[@]}")
...
if [[ "${1:-}" == "--reinstall-hooks" ]]; then
  ...
  if [[ -z "$PROJECT_DIR" ]]; then
    ...
    echo "   The user project must already have been initialized via /cheat-essay-init."
    exit 1
  fi
  ...
  if [[ ! -f "$PROJECT_DIR/.cheat-state.json" ]]; then
    echo "❌ $PROJECT_DIR is not a cheat-on-essay project (no .cheat-state.json)."
    echo "   Run /cheat-essay-init in that directory first."
    exit 1
  fi
  ...
  echo "  - rubric_notes.md / predictions/ / articles/ (your work)"
  echo ""
  echo "If schema also changed (CHANGELOG marks BREAKING), additionally run /cheat-essay-migrate"
...
# Sanity check: confirm we're in the cheat-on-essay root
if [[ ! -f "$SCRIPT_DIR/SKILL.md" ]]; then
  echo "❌ Missing: $SCRIPT_DIR/SKILL.md"
  echo "   Are you running install.sh from the cheat-on-essay root?"
  exit 1
fi
...
install_skills() {
  ...
  echo ""
  echo "Installing cheat-on-essay for $label (mode: $MODE)"
